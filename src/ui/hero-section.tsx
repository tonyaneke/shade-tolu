    ...
    <div className="flex gap-2 justify-center">
      {timeUnits.map((unit, index) => (
        <motion.div
          ...
          className="relative"
        >
          <div className="bg-white/15 backdrop-blur-xl rounded-xl border border-white/30 px-3 py-2 min-w-[60px] shadow-xl">
            <motion.div
              key={unit.value}
              initial={{ scale: 1.2, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              transition={{ duration: 0.3 }}
              className="text-2xl md:text-3xl font-bold text-white drop-shadow-lg font-playfair-display-sc"
            >
              {String(unit.value).padStart(2, "0")}
            </motion.div>
            <div className="text-[10px] text-amber-200 font-semibold uppercase tracking-wider mt-0.5">
              {unit.label}
            </div>
          </div>
          ...
          <motion.div
            ...
            className="absolute -inset-[2px] bg-gradient-to-br from-amber-300/40 to-yellow-300/40 rounded-xl blur-lg -z-10"
          />
        </motion.div>
      ))}
    </div>
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <p className="text-xl md:text-3xl font-bold mb-1 text-amber-200">
              December 26, 2025
            </p>
